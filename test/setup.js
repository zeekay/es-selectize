describe('Setup', function() {
    it('should not allow duplicate initialization', function() {
        var instanceBefore, instanceAfter, test

        test = setupTest('<input type="text">', {})
        instanceBefore = test.$select[0].selectize
        test.$select.selectize()
        instanceAfter = test.$select[0].selectize

        expect(instanceBefore).to.be.equal(instanceAfter)
    })

    describe('<input type="text">', function() {
        it('should complete without exceptions', function() {
            var test = setupTest('<input type="text">', {})
        })

        it('should populate items,options from "dataAttr" if available', function() {
            var data = [{val: 'a', lbl: 'Hello'}, {val: 'b', lbl: 'World'}]
            var test = setupTest('<input type="text" value="c,d,e" data-hydrate="' + JSON.stringify(data).replace(/"/g,'&quot;') + '">', {
        ...
        describe('getValue()', function() {
            it('should return value as a string', function() {
                var test = setupTest('<input type="text" value="a,b">', {delimiter: ','})
                expect(test.selectize.getValue()).to.be.a('string')
            })

            it('should return "" when empty', function() {
                var test = setupTest('<input type="text" value="">', {delimiter: ','})
        ...
        describe('<input type="text" attributes>', function() {
            it('should propagate original input attributes to the generated input', function() {
                var test = setupTest('<input type="text" autocorrect="off" autocapitalize="none">', {})
                expect(test.selectize.$controlInput.attr('autocorrect')).to.be.equal('off')
                expect(test.selectize.$controlInput.attr('autocapitalize')).to.be.equal('none')
            })

            it('should not add attributes if not present in the original', function() {
                var test = setupTest('<input type="text">', {})
                expect(test.selectize.$controlInput.attr('autocorrect')).to.be.equal(undefined)
                expect(test.selectize.$controlInput.attr('autocapitalize')).to.be.equal(undefined)
            })
        })
    })
    ...
    describe('<input type="number">', function() {
        it('should complete without exceptions', function(done) {
            var test = setupTest('<input type="number">', {})
            window.setTimeout(function() {
                assert.equal(test.selectize.$controlInput.attr('type'), 'number')
                done()
            }, 0)
        })
    })
    ...
    describe('<select>', function() {
        it('should complete without exceptions', function() {
            var test = setupTest('<select></select>', {})
        })
        ...
        it('should add options in text form (no html entities)', function() {
            var test = setupTest('<select><option selected value="a">&lt;hi&gt;</option></select>', {})
        ...
        it('should keep options in original order if no sort given', function(done) {
            ...
            var orderExpected = ['AL','AK','AZ','AR','CO','CT','DE','DC','FL','GA','HI','ID','IL','IN','IA','KS','KY','LA','ME','MD','MA','MI','MN','MS','MO','MT','NE','NV','NH','NJ','NM','NY','NC','ND','OH','OK','OR','PA','RI','SC','SD','TN','TX','UT','VT','VA','WA','WV','WI','01','10']
            var orderActual = []

            test.selectize.refreshOptions(true)
            window.setTimeout(function() {
                test.selectize.$dropdown.find('[data-value]').each(function(i, el) {
                    orderActual.push($(el).attr('data-value'))
                })

                expect(orderActual).to.eql(orderExpected)
                done()
            }, 0)
        })
        ...
        describe('getValue()', function() {
            ...
            it('should return proper value when not empty', function() {
                var test = setupTest('<select><option selected value="a">A</option></select>', {})
    ...
    describe('<select multiple>', function() {
        ...
        describe('getValue()', function() {
            ...
            it('should return proper value as array when not empty', function() {
                var test = setupTest('<select multiple><option selected value="a">A</option></select>', {})
    ...
    describe('<select disabled>', function() {
        ...
        before(function() {
            test = setupTest('<select disabled>', {})
        })
    ...
    describe('<select required>', function(){
        ...
        beforeEach(function() {
            test = setupTest('<select required>' +
        ...
        function(done) {
            ...
            $form.one('submit.test-required', function(e) {
                done()
            })
    ...
    describe('<select> (custom dom render)', function() {
        ...
        it('should render the custom option element', function(done) {
            ...
            window.setTimeout(function() {
                expect(test.selectize.$dropdownContent.find('.custom-option').length).to.be.equal(1)
                done()
            }, 0)
        })
    })
